...
**Never use plain `<img>` or Next.js `<Image>` directly for content images.** All visible content images go through `<ProtectedImage>` from `@/components/ProtectedImage`, which wraps `next/image` and blocks drag-and-drop, right-click, and drag-ghost preview.
...
3. If neither, the image's `src` is looked up in the build-time manifest at `src/lib/image-dimensions.json` (generated by `npm run image-manifest` and via the `prebuild` hook) → intrinsic mode using those dims.
